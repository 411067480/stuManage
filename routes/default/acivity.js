...
router.get('/activityContent/:id',async(ctx)=>{
    /*导航栏nav获取 */
    var nav = await DB.find('nav',{},{},{});
    var a=ctx.state.F;
    ...
    // //打印出被选择对象的id
    console.log(id);
    var currentResult = await DB.find('acivity',{'_id':DB.getObjectId(id)}); 
    //console.log(result[0].associationName);
    var allData = await DB.find('acivity',{},{},{});
    ctx.render('default/activityContent',{
        nav:nav,
